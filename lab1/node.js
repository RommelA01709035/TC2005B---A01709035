...
const server = http.createServer( (request, response) => {    
    ...
    if (request.url == "/") {
      ...
      response.write(`
        ...
        <h2>Si no quieres morir no presiones el boton</h2>
          <form action="/construir" method="POST">
            <input class="button is-warning" type="submit" value="Construir">
          </form>
      `);
      response.write(footer);
      response.end();
    ...
    else if (request.url == "/Enemigo" && request.method == "POST") {
    ...
    else if (request.url == "/Enemigo" && request.method == "GET") {

      response.write(header);
      response.write(Enemigo);
      response.write(footer);
      response.end();
